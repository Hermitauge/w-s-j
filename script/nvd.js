...
(() => {
  window.addEventListener('load', async () => {
          ...
          let currentPage = 1, limit = 30, fetching = false;
          ...
          function setupInfiniteScroll() {
            ...
            // Helper function to load more items when user scrolls
            async function loadMoreItems() {
              if (listContainer.scrollTop + listContainer.clientHeight >= listContainer.scrollHeight - 100 && !fetching) {
                fetching = true;
                showLoadingAnimation();
          
                const additionalProducts = await fetchProductsForFilters(
                  checkedShapes, 
      ...
      if (additionalProducts.length > 0) {
        await updateList(additionalProducts, listInstance, itemTemplateElement, true); // Append new items
        currentPage++; // Increment current page after successful fetch
      } else {
        // Handle no more items, if necessary
      }
  ...
  // Attach the load more items function to the scroll event with debounce
  listContainer.addEventListener('scroll', debounce(loadMoreItems, 300));
}
...
async function fetchProductsForFilters(checkedShapes, minPrice, maxPrice, minCarats, maxCarats, minColor, maxColor, minClarity, maxClarity, minCut, maxCut, checkedLabs, minPolish, maxPolish, minSymmetry, maxSymmetry, minFluor, maxFluor, minTable, maxTable, minDepth, maxDepth, minRatio, maxRatio, checkedOrigin, page, limit) {  
  let products = [];  
  ...
  return products;  
    ...
    async function applyAllFilters() {  
        ...
        const filteredProducts = await fetchProductsForFilters(checkedShapes, minPrice, maxPrice, minCarats, maxCarats, minColor, maxColor, minClarity, maxClarity, minCut, maxCut, checkedLabs, minPolish, maxPolish, minSymmetry, maxSymmetry, minFluor, maxFluor, minTable, maxTable, minDepth, maxDepth, minRatio, maxRatio, checkedOrigin, page, limit);  
  ...
  async function fetchProducts(shapeFilter = '', minPrice = '', maxPrice = '', minCarats = '', maxCarats = '', minColor = '', maxColor = '', minClarity = '', maxClarity = '', minCut = '', maxCut = '', checkedLabs = [], minPolish = '', maxPolish = '', minSymmetry = '', maxSymmetry = '', minFluor = '', maxFluor = '', minTable = '', maxTable = '', minDepth = '', maxDepth = '', minRatio = '', maxRatio = '', checkedOrigin = [], page, limit) {  
